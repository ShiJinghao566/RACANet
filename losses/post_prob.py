"""Build posterior maps from point annotations."""
...
class Post_Prob(Module):
    def __init__(self, sigma, c_size, stride, background_ratio, use_background, device):
        ...
        self.device = device
        # Coordinates are fixed because all crops share the same size.
    ...
    def forward(self, points, st_sizes):
        """Build point and background posteriors for each image."""
        ...
        if len(all_points) > 0:
            ...
            for dis, st_size in zip(dis_list, st_sizes):
                if len(dis) > 0:
                    if self.use_bg:
                        # Background grows with distance to the nearest point.
                    ...
                    # Normalize Gaussian likelihoods across points.
